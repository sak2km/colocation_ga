...
def plot_fitness_density(config: ColocationConfig, cache):
    ...
    for i in range(5):
        ...
        plot_histo(axes, np.array(cache["fitnesses"][iteration]).reshape(-1), bins)
        axes.axvline(truth_fitness, color="red")
        plt.tight_layout()
    if config.save_figure:
        fig2.savefig(config.base_file_name + "fitness_density_over_time.png", dpi=300)
...
@contextmanager
def visualizing(*, nrows=1, ncols=1, figsize=(6, 4), dpi=300, filename="", **kwargs):
