...
def load_matrix(matrix_file: str):
    ...
    """
    return io.read_file(matrix_file)


def select_rooms(matrix: np.ndarray, selected_rooms: List[int], type_count: int):
    ...
    """
    sensor_ids = np.array(
        [[r_id * type_count + j for j in range(type_count)] for r_id in selected_rooms]
    ).reshape(-1)
