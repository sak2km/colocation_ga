...
class ColocationConfig:
    ...
    def to_dict(self):
        """Generate a dict object based on the data class
        """
    ...
    def base_file_name(self):
        ...
        """
        return str(pathlib.Path(self.output_path).joinpath(self.job_name))

    def join_name(self, name: str):
        """Join the name based on a base name of the config
        """
        return pathlib.Path(self.base_file_name).joinpath(name)


def load_config(config_file_path: str, **overwrite_kwargs) -> ColocationConfig:
    ...
    for key, val in overwrite_kwargs.items():
        if val:
            config[key] = val
