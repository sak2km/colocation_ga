...
@numba.jit(numba.void(numba.int32[:], numba.int32, numba.float64, numba.float64[:]))
...
@numba.jit(numba.void(numba.int32[:], numba.int32, numba.float64))
def mutate_by_swap(population, child_id, mut_rate):
    """Perform mutation by randomly swapping pairs of rooms within a type.
    """
    solution = population[child_id]
    room_count, type_count = solution.shape
    pair_count = np.clip(
        np.random.poisson(mut_rate * room_count / 2, type_count), 0, room_count // 2
    )

    for type_id in range(type_count):
        count: int = pair_count[type_id]
        if count == 0:
            continue
        ids = np.random.choice(room_count, count * 2, replace=False)
        swap_i, swap_j = ids[:count], ids[count:]
        solution[swap_i, type_id] ^= solution[swap_j, type_id]
        solution[swap_j, type_id] ^= solution[swap_i, type_id]
        solution[swap_i, type_id] ^= solution[swap_j, type_id]
...
def initialize_population(population_size, room_count, type_count):
    ...
    """
    ind = np.arange(room_count * type_count, dtype=np.int32).reshape(
        1, room_count, type_count
    )
...
def survival_of_fittest(fitnesses, survivor_count, replaced_count):
...
def next_gen(
    population, survivor_ids, loser_ids, crossing_over_rate, mutation_rate, weight_func
):
    ...
    weights = np.ones(population.shape[1], dtype=np.float64) / population.shape[1]

    for i, l_id in enumerate(loser_ids):
        m_id, f_id = mothers[i], fathers[i]
        population[l_id] = population[m_id]

        if crossing_over_rate and f_id != m_id:
        ...
        if weight_func is not None:
            ...
            mutate_by_swap(population, l_id, mutation_rate)
