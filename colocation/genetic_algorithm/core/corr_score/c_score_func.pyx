...
cdef double c_solution_mean_4(double[:, :] corr_matrix, int[:, :] solution) nogil:
    ...
    with nogil, parallel():
        for i in prange(room_count, schedule='static'):
...
@cython.boundscheck(False)
cdef double c_solution_mean_ahu(
    double[:, :] corr_matrix, int[:,:] solution, int[:] vav_counts
) nogil:
    cdef int r_count = vav_counts.shape[0]
    cdef int start = 0
    cdef int r = 0
    cdef double total_coef = 0
    cdef int count = 0
    cdef int i, j, s_i, s_j
    cdef int v_count

    for r in range(r_count):
        v_count = vav_counts[r]
        for i in range(v_count - 1):
            s_i = solution[start + i, 0]
            for j in range(i + 1, v_count):
                s_j = solution[start + j, 0]
                count += 1
                total_coef += corr_matrix[s_i, s_j]
        
        for i in range(v_count):
            s_i = solution[start + i, 0]
            count += 1
            total_coef += corr_matrix[r, s_i]
...
cpdef solution_mean_ahu(double[:, :] corr_matrix, int[:,:] solution, int[:] vav_counts):
    """Calculate mean coefficients for ahu
    """
    return c_solution_mean_ahu(corr_matrix, solution, vav_counts)
